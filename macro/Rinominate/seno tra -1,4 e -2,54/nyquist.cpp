...
void nyquist(const char *nomeFile, bool verbose=true)
{
    ...
    double err_codici[521], err_tempi[521], zeri[521];
    for (unsigned int i = 0; i < 521; i++)
    {
...
*/
    ...
    TGraphErrors *g0 = new TGraphErrors(521, &tempi[0], &codici[0], zeri, &err_codici[0]);
    const char *func = "[0]*TMath::Sin(TMath::TwoPi()*[1]*x+[2])+[3]";
    TF1 *fit = new TF1("fit", func, 0., TMath::MaxElement(521, &tempi[0]) + 1);
    fit->SetNpx(10 * 521); // Miglioramento risoluzione del grafico
    ...
    for (unsigned int i = 0; i < frequenzeMigliori.size(); i++)
    {
        ...
        TGraphErrors *g = new TGraphErrors(521, &tempi[0], &codici[0], zeri, &err_codici[0]);
